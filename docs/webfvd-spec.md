...
- **Don't chase the last cm of FVD++ parity through float emulation alone.** A per-vertex drift profile (`scripts/drift-profile.ts`) on the worst corpus files traces the residual drift to `Subfunc.getValue` for higher-degree subfuncs, NOT to the rotation primitive: in `geo-degree-yaw`, Linear and Quadratic sections drift <0.01 mm over their full 15 m; the drift starts at Cubic (1.4 mm), grows monotonically with polynomial order (Quintic 11 mm, Sinusoidal 13 mm, ToZero 20 mm), and explodes in Freeform (75 mm at section end — though most of that is inherited from upstream sections). The likely cause: FVD's C++ runs the polynomial chain in float32 with intermediate rounding at every `*`, while JS evaluates the whole expression in float64 and rounds once at the `r(...)` boundary. Closing this fully would require either replacing every polynomial branch with a hand-emulated float32 chain (high risk of regressing the well-tuned `testtrack` parity — the `geo-arg1` experiment showed Quartic/Quintic prefer the float64 path) or an accumulated-quaternion refactor of the Geometric integrator (~200 LoC). **Plan instead:** ship the present integrator as the "FVD-compatible" mode and offer a precise opt-in mode later (see §6.6.X) that uses float64 throughout and trades file-format byte parity for sub-mm accuracy — it's the right place to also add better integration schemes (RK4, adaptive) that we deliberately avoid in the compatible mode. Note: the `vec3RotateAxisGlm` cross-product intermediates are NOT rounded per-op — experiments adding `r()` after each multiply regressed `testtrack` parity 18×, because gcc keeps those intermediates in extended precision (x87 80-bit or FMA-fused) and the single round-at-end matches better.
