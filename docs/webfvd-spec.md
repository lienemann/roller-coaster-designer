...
- **FVD++ float-emulation campaign — findings so far (read before touching `r()`/`f()` placement).** The reference binary is FVD++ 0.79 for Windows, almost certainly MinGW i686 → x87 FPU, `FLT_EVAL_METHOD == 2`, gcc -O2: float expressions AND float locals live in 80-bit registers and round to float32 only at memory stores (class members, arrays) and non-inlined call boundaries. JS doubles are the right proxy for the 80-bit intermediates. Hard-won experimental results, all measured against the 11 FVD++-generated NL2 goldens plus the testtrack byte oracle:
