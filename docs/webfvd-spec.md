...
- **Stereoscopic edit mode (SBS glasses) → post-T3.** A full-screen edit mode that renders the entire app side-by-side for stereo glasses (Viture Luma-class, XReal-class — peripherals that present as a single external monitor in SBS-3D mode, no WebXR runtime).
  - **3D viewport:** Three.js `StereoCamera` + scissored L/R viewports on a single full-screen canvas. Cheap (~100 lines).
  - **HTML/CSS UI:** the genuinely hard part. The browser draws each DOM node once; there's no "clone this subtree to the right half" primitive. Approach: **two iframes** of the same app URL, side-by-side; a shared store (Zustand + `BroadcastChannel` or `SharedWorker`) so edits land once and both iframes re-render. Mouse events fire only in the left iframe; the right iframe is a passive mirror. Other approaches considered and rejected: single tree with a CSS portal-clone (doubles every observer/event), UI-on-canvas (loses accessibility + DOM testing).
  - **Cursor:** browsers don't draw the OS cursor inside a canvas. Solution: `pointer-lock`, maintain a virtual cursor in a store slice, software-draw a small SVG cursor at the virtual position in both halves, hit-test UI against the virtual position.
  - **Refresh rate:** Luma-class glasses drop to 60 Hz in SBS. Plan viewport animation budgets accordingly; the worker integrator is unaffected.
  - **Scope estimate:** 1–2 weeks. Worth doing only when there's evidence users would edit for hours in stereo; otherwise the simpler "Spacewalker browser as a 2D floating screen" workflow covers the common case at zero engineering cost.
...
- **FVD++ float-emulation campaign — findings so far (read before touching `r()`/`f()` placement).** PLATFORM CORRECTION (supersedes the x87 model below): the shipped binary `reference/openfvd/bin/win64/FVD.exe` — the source of all golds — is x86-64, MSYS2 MinGW-w64 GCC 6.2.0, Qt 5.6.2, glm ≈ 0.9.8.0, i.e. SSE2 math with `FLT_EVAL_METHOD == 0`: float expressions round to float32 PER OPERATION. The earlier working model (MinGW i686 x87, 80-bit intermediates, round-at-store) was inferred and is wrong; the committed r() placement encodes measured fits to that wrong model and should migrate to per-op SSE2 semantics validated against `tools/fvd-oracle` node dumps (see docs/parity-campaign.md). Hard-won experimental results, all measured against the 11 FVD++-generated NL2 goldens plus the testtrack byte oracle:
  1. Strict per-op float32 rounding (the SSE2 model) in `Subfunc.getValue` REGRESSES `geo-arg1` 0.25→16 mm and `geo-kinematics` 0.5→32 mm. The committed double-inner/round-at-return code matches the x87 register model and stays.
  2. `TO_RAD` in C++ is a SINGLE multiply by float-constant `F_RAD = 0.0174532925199432958f` (lenassert.h:32), and `F_PI` is the FLOAT value 3.1415927410125732 — our `Math.PI`-based `toRad` is formally wrong. BUT applying either fix flips discrete integrator branches (`|artificialRoll| ≥ 90` sign, per-section node counts, lead-out break conditions) and the whole-file maxima jump chaotically: F_PI=π32 alone takes kinematics 0.5→32 mm while improving warp 58→40. Partial fixes are WORSE than the consistent baseline.
  3. Therefore: do not land single-constant fixes. The route to "loaded FVD++ tracks keep their carefully matched ends" is a simultaneous-correctness campaign: (a) port each integrator hot-loop site with x87 promotion semantics derived from the C++ expression types; (b) instrument the FIRST diverging node per section (the drift profiles in `scripts/drift-profile.ts` localise it) and bisect the upstream quantity bit-by-bit; (c) gate each change on the FULL corpus + testtrack bytes, accepting only Pareto improvements. Track the branch knife-edges explicitly — log `sign` flips and per-section node counts and compare against the gold's vertex spacing.
  4. Current baseline (the committed state): freeform 0.00, roll 0.10, multisub 0.11, length-threshold 0.28, trig-isolation 0.47, options 1.41, kinematics 1.62, arg1 3.29, pitch 7.1, warp 19.1, yaw 28.8 mm; testtrack first-write parity = 2 low-mantissa bytes (≤25 ULP, both the curved-boundary vLat.y residual). Key applied semantics: inline-extended getPitch/getDirection in calcDirFromLast (the section-start `translateValues(fYawFromLast*F_HZ)` anchor amplifies one heading ULP ×1000 into a whole-section rate offset — the dominant drift mechanism), TO_RAD = deg·float32(F_RAD), float32 F_G (with the bare-double 9.80665 counter-quirk in the geometric/bezier energy sqrt), cross-TU float-parameter rounding (func::getValue arg, setRoll/changePitch/changeYaw angles), glm-0.9.5.1 normalize (dot and sqrt round at the sqrtf boundary), float32 Thomas solve in the NL2 exporter. Measured rejections (don't re-apply blind): global F_PI=π32, glm source-form quat·vec3 (gcc SRA keeps uv/uuv in registers), fround(sin) as the sinf model. Full detail: docs/parity-campaign.md.
- **Don't add per-op rounding inside Curved/Forced/Straight `vPos +=` updates.** Only the Geometric TIME and DISTANCE integrators benefit from rounding the per-step `vPos` write to float32. The same change in `sec-curved.ts` / `sec-forced.ts` / `sec-straight.ts` regresses `testtrack` parity from 1.4 mm to ~26 mm — apparently FVD's accumulator in those integrators carries sub-ULP detail that matches better when JS keeps the double-precision sum.
